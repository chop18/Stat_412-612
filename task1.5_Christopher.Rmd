...
main_covid_df %>%   
  group_by(state_name) %>% 
   mutate(cohort1 = case_when(stay_home_restrictions !=1 & date<=first_date_SH1 & date>=(first_date_SH1-days(14))~"a",
                              stay_home_restrictions ==1 & date<=date_pluse_30_SH1 & date>=(date_pluse_30_SH1-days(14))~"b",
                             date>first_date_SH1 | date<(date_pluse_30_SH1-days(14)) | date<(first_date_SH1-days(14)) | date>date_pluse_30_SH1  ~"None"),
         cohort2= case_when(stay_home_restrictions !=2 & date<=first_date_SH2 & date<(first_date_SH2-days(14))~"a",
                              stay_home_restrictions ==2 & date<=date_pluse_30_SH2 & date>=(date_pluse_30_SH2-days(14))~"b",
                              date>first_date_SH2 | date<(date_pluse_30_SH1-days(14)) | date<(first_date_SH2-days(14)) | date>date_pluse_30_SH2 ~"None")) %>% 
group_by(cohort1, state_name) %>% 
  mutate(daily_cases = c(0,diff(confirmed)),
         rate= c(0,diff(daily_cases))) %>% 
  summarise(mean = mean(daily_cases))
```
